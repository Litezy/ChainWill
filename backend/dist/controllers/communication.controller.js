...
function validateNotificationPayload(body) {
        ...
        return {
            ...
            beneficiaries: payload.beneficiaries.map((beneficiary, index) => {
                if (!isObject(beneficiary)) {
                    throw new ValidationError(`beneficiaries[${index}] must be an object`);
                }
                return {
                    beneficiaryName: requireString(beneficiary.beneficiaryName, `beneficiaries[${index}].beneficiaryName`),
                    beneficiaryEmail: requireEmail(beneficiary.beneficiaryEmail, `beneficiaries[${index}].beneficiaryEmail`),
                    allocationPercentage: requireAllocation(beneficiary.allocationPercentage),
                };
            }),
            ownerName: requireString(payload.ownerName, "ownerName"),
            contractAddress: requireContractAddress(payload.contractAddress, "contractAddress"),
    ...
    return {
        ...
        signers: payload.signers.map((signer, index) => {
            ...
            return {
                signerName: requireString(signer.signerName, `signers[${index}].signerName`),
                signerEmail: requireEmail(signer.signerEmail, `signers[${index}].signerEmail`),
            };
        }),
        ownerName: requireString(payload.ownerName, "ownerName"),
        contractAddress: requireContractAddress(payload.contractAddress, "contractAddress"),
...
async function sendNotificationEmail(req, res) {
    try {
        ...
        if (payload.type === "owner") {
            ...
            await Promise.all(payload.beneficiaries.map((beneficiary) => mailerService_1.mailerService.sendBeneficiaryClaimEmail({
                ...beneficiary,
            ...
            await Promise.all(payload.signers.map((signer) => mailerService_1.mailerService.sendSignerReminderEmail({
                ...signer,
                ownerName: payload.ownerName,
                contractAddress: payload.contractAddress,

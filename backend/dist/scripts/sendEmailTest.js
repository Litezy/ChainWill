...
async function main() {
    ...
    console.log(`[EmailTest] Delivery mode: ${process.env.SMTP_HOST ? 'smtp' : 'preview'}`);
    await mailerService_1.mailerService.sendOwnerWillCreatedEmail({
        ownerName,
    ...
    await mailerService_1.mailerService.sendSignerReminderEmail({
    ...
    await mailerService_1.mailerService.sendBeneficiaryClaimEmail({
    ...
    await mailerService_1.mailerService.sendVerificationOtpEmail({
        ...
        expiresInMinutes: 10,
        contractAddress,
        verificationPageUrl: `${process.env.APP_FRONTEND_URL || 'http://localhost:5173'}/claim-inheritance/${encodeURIComponent(recipientEmail)}`,

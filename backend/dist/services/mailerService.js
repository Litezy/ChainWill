...
exports.mailerService = exports.MailerService = void 0;
const dotenv_1 = __importDefault(require("dotenv"));
const handlebars_1 = __importDefault(require("handlebars"));
const nodemailer_1 = __importDefault(require("nodemailer"));
dotenv_1.default.config();
const handlebarsInstance = handlebars_1.default.create();
const baseHtmlTemplate = handlebarsInstance.compile(`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
const baseTextTemplate = handlebarsInstance.compile(`{{heading}}
...
const ownerWillCreatedTemplate = {
    subject: handlebarsInstance.compile('Congratulations {{ownerName}}, your ChainWill has been created'),
    previewText: handlebarsInstance.compile('Your will is now live on-chain. Save your contract address and review the next steps.'),
...
const signerReminderTemplate = {
    subject: handlebarsInstance.compile('Reminder: please sign the ChainWill attestation for {{ownerName}}'),
    previewText: handlebarsInstance.compile('A signer action is pending. Open the secure signing page and complete your attestation.'),
...
const beneficiaryClaimTemplate = {
    subject: handlebarsInstance.compile('Your ChainWill inheritance claim for {{allocationPercentageLabel}} is ready'),
    previewText: handlebarsInstance.compile('Your beneficiary allocation is ready to be reviewed and claimed through the secure claim page.'),
...
const verificationOtpTemplate = {
    subject: handlebarsInstance.compile('Your ChainWill verification code for {{audienceLabel}} access'),
    previewText: handlebarsInstance.compile('Use this one-time code to verify your email and continue your secure ChainWill session.'),
...
class MailerService {
    ...
    replyTo = process.env.MAIL_REPLY_TO || process.env.ALERT_EMAIL_REPLY_TO;
    supportEmail = process.env.MAIL_SUPPORT_EMAIL ||
        this.replyTo ||
        'support@chainwill.local';
    frontendBaseUrl = process.env.APP_FRONTEND_URL || 'https://chain-will.vercel.app';
    transporter;
    previewTransport;
    constructor() {
    ...
    renderOwnerWillCreatedEmail(input) {
        return this.renderTemplate(ownerWillCreatedTemplate, {
            ownerName: input.ownerName,
            contractAddress: input.contractAddress,
            dashboardUrl: input.dashboardUrl || this.buildAdminDashboardUrl(input.contractAddress),
            explorerUrl: input.explorerUrl,
        }, input.supportEmail);
    }
    async sendOwnerWillCreatedEmail(input) {
    ...
    renderBeneficiaryClaimEmail(input) {
        return this.renderTemplate(beneficiaryClaimTemplate, {
            ...
            contractAddress: input.contractAddress,
            claimPageUrl: input.claimPageUrl ||
                this.buildClaimPageUrl(input.beneficiaryEmail, input.contractAddress),
        }, input.supportEmail);
    }
    async sendBeneficiaryClaimEmail(input) {
    ...
    renderVerificationOtpEmail(input) {
        return this.renderTemplate(verificationOtpTemplate, {
            ...
            verificationPageUrl: input.verificationPageUrl ||
                this.buildVerificationPageUrl(input.recipientEmail, input.audience, input.contractAddress),
            purpose: input.purpose,
        }, input.supportEmail);
    }
    async sendVerificationOtpEmail(input) {
    ...
    renderTemplate(template, context, supportEmailOverride) {
        const layoutContext = {
    ...
    buildSigningPageUrl(email, contractAddress) {
        const url = new URL(`/sign-inheritance/${encodeURIComponent(email)}`, this.frontendBaseUrl);
        url.searchParams.set('contract', contractAddress);
        return url.toString();
    }
    buildClaimPageUrl(email, contractAddress) {
        const url = new URL(`/claim-inheritance/${encodeURIComponent(email)}`, this.frontendBaseUrl);
        url.searchParams.set('contract', contractAddress);
        return url.toString();
    }
    ...
    buildVerificationPageUrl(email, audience, contractAddress) {
        if (audience === 'signer') {
            return contractAddress
                ...
                : new URL(`/sign-inheritance/${encodeURIComponent(email)}`, this.frontendBaseUrl).toString();
        }
        return contractAddress
            ? this.buildClaimPageUrl(email, contractAddress)
            : new URL(`/claim-inheritance/${encodeURIComponent(email)}`, this.frontendBaseUrl).toString();
    }
}
